...
let items = document.querySelectorAll('.itemId');
let lastIndex = items[items.length - 1].innerText
...
const wrapper = document.getElementById('cards-holder');
...
showMore.addEventListener('click', function () {
    $.ajax({
        ...
        success: function (html) {
            ...
            html.forEach(item => {
                wrapper.innerHTML += `
                <a href="advertisementDetail.php?advertisement=${item.id}" class="card">
                        <div class="itemId" style="display: none">${item.id}</div>
                        <img class="card-img" src="${item.photopath}"/>
            ...
            lastIndex = items[items.length - 1].innerText
        }
    })
})
